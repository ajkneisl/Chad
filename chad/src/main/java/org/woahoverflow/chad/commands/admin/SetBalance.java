    ...
    public Runnable run(MessageReceivedEvent e, List<String> args) {
        return () -> {
            ...
            if (args.size() == 1)
            {
                ...
                ChadVar.DATABASE_DEVICE.set(e.getGuild(), e.getAuthor().getStringID() + "_balance", Integer.parseInt(args.get(0)));
                new MessageHandler(e.getChannel()).send("Set your balance to `"+args.get(0)+"`.", "Balance");
                return;
            }
            if (args.size() == 2)
            {
                if (e.getMessage().getMentions().contains(0))
                {
                ...
                ChadVar.DATABASE_DEVICE.set(e.getGuild(), e.getMessage().getMentions().get(0).getStringID() + "_balance", Integer.parseInt(args.get(0)));
